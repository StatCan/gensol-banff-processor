...
Des exemples supplémentaires peuvent être trouvés [ici](../../examples).


<h2 id="outil-de-generation-de-metadonnees">Outil de génération de métadonnées</h2>
...
<h2 id="parametres-d-entree-du-processeur-banff">Paramètres d'entrée du processeur Banff</h2>
...
|auxdata_filename|Le nom de fichier ou le chemin d'accès complet de votre fichier de données auxiliaires|N|
|histdata_filename|Le nom de fichier ou le chemin d'accès complet de votre fichier de données historiques|N|
|histstatus_filename|Le nom de fichier ou le chemin d'accès complet de votre fichier de données d'état historique|N|
...
|log_level|Configure la création ou non d'un fichier journal et le niveau de messages qu'il doit contenir. La valeur doit être 0, 1 (par défaut) ou 2. Voir [Output](#output).|N|
...
<h2 id="execution-du-processeur-en-tant-qu-utilitaire-de-ligne-de-commande">Exécution du processeur en tant qu'utilitaire de ligne de commande</h2>
...
<h2 id="blocs-de-processus">Blocs de processus</h2>
